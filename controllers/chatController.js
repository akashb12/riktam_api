...
const fetchUserChats = async (req, res) => {
    try {
        let query = { users: { $elemMatch: { $eq: req.user.id } } };
        if (req.query.searchName) {
            // Add search name condition to the query
            query = {
                ...query,
                "chatName": { $regex: req.query.searchName, $options: 'i' }
            };
        }
...
const deleteGroup = async (req, res) => {
    try {
        await Message.findOneAndDelete({ chat: req.params.chatId });
